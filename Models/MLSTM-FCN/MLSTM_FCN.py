...
class MLSTM_FCN(nn.Module):
    ...
    def init_hidden(self):
        h0 = torch.zeros(self.N_LSTM_layers, self.Batch_Size, self.N_LSTM_Out)
        c0 = torch.zeros(self.N_LSTM_layers, self.Batch_Size, self.N_LSTM_Out)
        return h0, c0
...
if __name__ == "__main__":
    model = MLSTM_FCN(32, 128, 4)
    print(model)

    x = torch.randn(32, 128, 200)
    y = model(x)
    print(y.shape)